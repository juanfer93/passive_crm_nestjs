...
export class WhatsappWebClientService implements OnModuleInit, OnModuleDestroy {
  ...
  private async handleIncomingMessage(message: Message): Promise<void> {
    ...
    this.publishVivaSofiaEvents(
      ...
      isNewLead,
      this.receivedDocumentFile(kind),
    );
  }
  ...
  private publishVivaSofiaEvents(
    ...
    this.background.run('viva-sofia-event-publisher', async () => {
      ...
      const basePayload = {
        ...
        phone: leadFields.phone ?? state?.customerProfile?.phone ?? this.phoneFromJid(contactId),
  ...
  private receivedDocumentFile(kind: ConversationMessageKind): boolean {
